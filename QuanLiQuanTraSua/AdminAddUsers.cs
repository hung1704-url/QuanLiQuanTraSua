...
namespace QuanLiQuanTraSua
{
    public partial class AdminAddUsers : UserControl
    {
        ...
        private void adminAddUsers_importBtn_Click(object sender, EventArgs e)
        {
            if(emptyFields())
            {
                MessageBox.Show(" tat ca cac o can duoc dien ", " thong bao loi ", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                if( connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        using (SqlCommand checkUsern = new SqlCommand(selectUsern, connect))
                        {
                            ...
                            if(table.Rows.Count >=1)
                            {
                                ...
                                MessageBox.Show(usern + " da duoc chon roi  " , " thong bao loi ", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                            else
                            {
                                ...
                                string path = Path.Combine(@"D:\QuanLiQuanTraSua\QuanLiQuanTraSua\User_Directory\"+ adminAddUsers_username.Text.Trim() + ".jpg");
                    ...
                    {
                        MessageBox.Show(" ket noi that bai " + ex, " thong bao loi ", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }finally 
                    {
                        connect.Close();
        ...
        private void import_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("loi" + ex, "thong bao loi ", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void adminAddUsers_updateBtn_Click(object sender, EventArgs e)
        {
            if (emptyFields())
            {
                MessageBox.Show(" tat ca can duoc dien ", "thong bao loi ", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    ...
                    if (connect.State != ConnectionState.Open)
                    {
                        try
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand(updateData, connect))
                            {
                                ...
                                cmd.ExecuteNonQuery();
                                clearFields();
                                MessageBox.Show("Updated thanh cong!", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                displayUsersData();
                            }
                        }
                        catch (Exception ex)
                        {
                            MessageBox.Show("ket noi that bai" + ex, "thong bao loi ", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        finally
                        {
                            connect.Close();
